...
*/
	if (isset($_SESSION["name"])) {
		...
		$searchquery="SELECT name FROM characters WHERE name='" . $_SESSION['name'] . "'";
		//If there are no results, insert.
		if (mysql_num_rows(mysql_query($searchquery))==0) {
		
			$runthisquery="INSERT INTO characters VALUES (NULL, '$name', '" . $_SESSION['hp'] . "', '" . $_SESSION['maxhp'] . "', '" . $_SESSION['level'] . "',  '" . $_SESSION['strength'] . "', '" . $_SESSION['defense'] . "', '" .  $_SESSION['speed'] . "', '" . $_SESSION['weaponholster'][0][0] . "',";
			...
			//in slot 0.
			if (isset($_SESSION['weaponholster'][1][0])) {
				$runthisquery.=" '" . $_SESSION['weaponholster'][1][0] . "',"; 
			}
			else {
				$runthisquery.=" NULL,";
			}
			if (isset($_SESSION['weaponholster'][2][0])) {
				$runthisquery.=" '" . $_SESSION['weaponholster'][2][0] . "',";
			}
			else {
				$runthisquery.=" NULL,";
			}
			//First Shield
			$runthisquery.=" '" . $_SESSION['shieldholster'][0][0] . "',";
			//Then the second and third slots
			if (isset($_SESSION['shieldholster'][1][0])) {
				$runthisquery.=" '" . $_SESSION['shieldholster'][1][0] . "',"; 
			}
			else {
				$runthisquery.=" NULL,";
			}
			if (isset($_SESSION['shieldholster'][2][0])) {
				$runthisquery.=" '" . $_SESSION['shieldholster'][2][0] . "',";
			}
			else {
				$runthisquery.=" NULL,";
			}
			
			$runthisquery.=" '" . $_SESSION['username'] . "', 'yes', '" . $_SESSION['exp'] . "', '" . $_SESSION['maxexp'] . "', '" . $_SESSION['numberofpotionholsterslots'] . "', '" . $_SESSION['numberofitemsinpotionholster'] . "',";
			
			//If there are potions in the potion holster, record them. If not, insert NULL.
			//Ends with a comma (,)
			$x=0;
			while (isset($_SESSION['potionholster'][$x][0])) {
				$runthisquery.=" '" . $_SESSION['potionholster'][$x][0] . "',";
				$x++;
			}
			...
			//Ends with a comma(,)
			$x=0;
			while (isset($_SESSION['itemholster'][$x][0])) {
				$runthisquery.=" '" . $_SESSION['itemholster'][$x][0] . "',";
				$x++;
			}
			...
			$runthisquery.=" '" . $_SESSION['weaponequipped'] . "', '" . $_SESSION['shieldequipped'] . "', '" . $_SESSION['gold'] . "'";
			//Quest Stuff
		...
		//If there is exactly 1 result, update
			...
			$runthisquery="UPDATE characters SET hp='" . $_SESSION['hp'] . "', maxhp='" . $_SESSION['maxhp'] . "', level='" . $_SESSION['level'] . "',  strength='" . $_SESSION['strength'] . "', defense='" . $_SESSION['defense'] . "', speed='" .  $_SESSION['speed'] . "', weapon1='" . $_SESSION['weaponholster'][0][4] . "',";
			//(characterid, name, hp, maxhp, level, strength, defense, weapon1, weapon2, weapon3, shield1, shield2, shield3, username, dead, exp, maxexp, numberofpotionholsterslots, numberofitemsinpotionholster, potion1, potion2, potion3, potion4, potion5, item1, item2, item3, item4, item5, weaponequipped, shieldequipped, gold)
			...
			$runthisquery.=" dead='yes', exp='" . $_SESSION['exp'] . "', maxexp='" . $_SESSION['maxexp'] . "',";
